...
public final class PushRequest extends FutureTask<PushResult> {
    ...
    private void sendToConnServer(RemoteRouter remoteRouter) {
        ...
        //2.通过网关连接，把消息发送到所在机器
        boolean success = elionClient.getGatewayConnectionFactory().send(
                ...
                pushMessage -> {
                    ...
                    future = elionClient.getPushRequestBus().put(sessionId, PushRequest.this);
                }
        );
        ...
        if (this.status.compareAndSet(Status.init, status)) {//防止重复调用
            ...
            if (callback != null) {//回调callback
                if (isTimeoutEnd) {//超时结束时，当前线程已经是线程池里的线程，直接调用callback
                    ...
                    elionClient.getPushRequestBus().asyncCall(this);//会执行run方法
                }
            }
        }
    ...
    public FutureTask<PushResult> broadcast() {
        ...
        boolean success = elionClient.getGatewayConnectionFactory()
                .broadcast(
                        connection -> GatewayPushMessage
                                .build(connection)
                                .setUserId(userId)
    ...
    private void offline() {
        elionClient.getCachedRemoteRouterManager().invalidateLocalCache(userId);
        submit(Status.offline);
    }

    private void timeout() {
        //超时要把request从队列中移除，其他情况是XXHandler中移除的
        if (elionClient.getPushRequestBus().getAndRemove(sessionId) != null) {
    ...
    public void onRedirect() {
        ...
        if (status.get() == Status.init) {//init表示任务还没有完成，还可以重新发送
            ...
            //3. 取最新的路由重发一次
            send(elionClient.getCachedRemoteRouterManager().lookup(userId, location.getClientType()));
        }
    }
    ...
    public PushRequest(ElionClient elionClient) {
        super(NONE);
        this.elionClient = elionClient;
    }

    public static PushRequest build(ElionClient elionClient, PushContext ctx) {
        ...
        return new PushRequest(elionClient)
