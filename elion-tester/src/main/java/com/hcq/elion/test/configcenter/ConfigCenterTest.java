...
package com.hcq.elion.test.configcenter;
...
public class ConfigCenterTest {
    ...
    public void testKey() {
        ...
        System.out.println(CC.elion.push.flow_control.global.max);
    }
    ...
    public static void print(String s, ConfigValue configValue, Map<String, String> map) {
        if (s.startsWith("elion") && !s.endsWith("\"")) {
            String[] keys = s.split("\\.");
    ...
    public void testLoad2() {
        Map<String, String> map = new HashMap<>();
        System.out.println("public interface elion {");
        System.out.printf("  Config cfg = ConfigManager.I.elion().toConfig();%n%n");
        CC.elion.cfg.root().forEach((s, configValue) -> print2(s, configValue, "elion", 1));
        System.out.println("}");
    }
