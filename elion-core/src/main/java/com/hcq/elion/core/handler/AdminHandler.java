...
public final class AdminHandler extends SimpleChannelInboundHandler<String> {
    ...
    public AdminHandler(ElionServer elionServer) {
    ...
    public void init() {
        register("help", (ctx, args) ->
                "Option                               Description" + EOL +
                        ...
                        "shutdown                             stop elion server" + EOL +
                        "restart                              restart elion server" + EOL +
        ...
        register("count", (ctx, args) -> {
            switch (args) {
                case "conn":
                    return elionServer.getConnectionServer().getConnectionManager().getConnNum();
                case "online": {
                    return elionServer.getRouterCenter().getUserEventConsumer().getUserManager().getOnlineUserNum();
                }
        ...
        register("route", (ctx, args) -> {
            ...
            Set<RemoteRouter> routers = elionServer.getRouterCenter().getRemoteRouterManager().lookupAll(args);
    ...
    public void channelActive(ChannelHandlerContext ctx) throws Exception {
        ctx.write("Welcome to elion Console [" + ConfigTools.getLocalIp() + "]!" + EOL);
