...
import com.hcq.elion.core.handler.*;
...
import static com.hcq.elion.tools.config.CC.elion.net.connect_server_bind_ip;
import static com.hcq.elion.tools.config.CC.elion.net.connect_server_port;
import static com.hcq.elion.tools.config.CC.elion.net.traffic_shaping.gateway_client.*;
import static com.hcq.elion.tools.config.CC.elion.net.write_buffer_water_mark.connect_server_high;
import static com.hcq.elion.tools.config.CC.elion.net.write_buffer_water_mark.connect_server_low;
...
public final class ConnectionServer extends NettyTCPServer {
    ...
    public ConnectionServer(ElionServer elionServer) {
    ...
    public void init() {
        ...
        messageDispatcher.register(Command.FAST_CONNECT, () -> new FastConnectHandler(elionServer));
        ...
        messageDispatcher.register(Command.HTTP_PROXY, () -> new HttpProxyHandler(elionServer), CC.elion.http.proxy_enabled);

        if (CC.elion.net.traffic_shaping.connect_server.enabled) {//启用流量整形，限流
    ...
    public void start(Listener listener) {
        ...
        if (this.workerGroup != null) {// 增加线程池监控
            elionServer.getMonitor().monitor("conn-worker", this.workerGroup);
        }
    }
    ...
    protected int getWorkThreadNum() {
        return CC.elion.thread.pool.conn_work;
    }
