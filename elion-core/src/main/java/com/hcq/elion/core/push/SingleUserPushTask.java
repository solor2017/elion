...
import com.hcq.elion.api.message.Message;
...
import com.hcq.elion.core.router.LocalRouter;
...
public final class SingleUserPushTask implements PushTask, ChannelFutureListener {
    ...
    public SingleUserPushTask(ElionServer elionServer, IPushMessage message, FlowControl flowControl) {
    ...
    private boolean checkTimeout() {
        if (start > 0) {
            if (System.currentTimeMillis() - start > message.getTimeoutMills()) {
                ...
                elionServer.getPushCenter().getPushListener().onTimeout(message, timeLine.timeoutEnd().getTimePoints());
    ...
    private boolean checkLocal(IPushMessage message) {
        ...
        LocalRouter localRouter = elionServer.getRouterCenter().getLocalRouterManager().lookup(userId, clientType);
        ...
        if (!connection.isConnected()) {
            ...
            //删除已经失效的本地路由
            elionServer.getRouterCenter().getLocalRouterManager().unRegister(userId, clientType);
        ...
        if (!connection.getChannel().isWritable()) {
            elionServer.getPushCenter().getPushListener().onFailure(message, timeLine.failureEnd().getTimePoints());
        ...
        if (flowControl.checkQps()) {
            ...
            elionServer.getPushCenter().delayTask(flowControl.getDelay(), this);
        }
        return true;
    }
    ...
    private void checkRemote(IPushMessage message) {
        String userId = message.getUserId();
        int clientType = message.getClientType();
        RemoteRouter remoteRouter = elionServer.getRouterCenter().getRemoteRouterManager().lookup(userId, clientType);

        // 1.如果远程路由信息也不存在, 说明用户此时不在线，
        if (remoteRouter == null || remoteRouter.isOffline()) {

            elionServer.getPushCenter().getPushListener().onOffline(message, timeLine.end("offline-end").getTimePoints());
        ...
        if (remoteRouter.getRouteValue().isThisMachine(elionServer.getGatewayServerNode().getHost(), elionServer.getGatewayServerNode().getPort())) {

            elionServer.getPushCenter().getPushListener().onOffline(message, timeLine.end("offline-end").getTimePoints());

            //删除失效的远程缓存
            elionServer.getRouterCenter().getRemoteRouterManager().unRegister(userId, clientType);
        ...
        elionServer.getPushCenter().getPushListener().onRedirect(message, timeLine.end("redirect-end").getTimePoints());
    ...
    public void operationComplete(ChannelFuture future) throws Exception {
        ...
        if (future.isSuccess()) {//推送成功
            ...
            if (message.isNeedAck()) {//需要客户端ACK, 添加等待客户端响应ACK的任务
                ...
                elionServer.getPushCenter().getPushListener().onSuccess(message, timeLine.successEnd().getTimePoints());
            }
    ...
    private void addAckTask(int messageId) {
        ...
        AckTask task = AckTask
                .from(messageId)
                .setCallback(new PushAckCallback(message, timeLine, elionServer.getPushCenter()));

        elionServer.getPushCenter().getAckTaskQueue().add(task, message.getTimeoutMills() - (int) (System.currentTimeMillis() - start));
    }
}
