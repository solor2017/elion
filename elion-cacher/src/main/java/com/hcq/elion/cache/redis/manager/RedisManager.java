...
import com.hcq.elion.cache.redis.connection.RedisConnectionFactory;
...
public final class RedisManager implements CacheManager {
    ...
    public void init() {
        ...
        factory.setPassword(CC.elion.redis.password);
        factory.setPoolConfig(CC.elion.redis.getPoolConfig(JedisPoolConfig.class));
        factory.setRedisServers(CC.elion.redis.nodes);
        factory.setCluster(CC.elion.redis.isCluster());
        if (CC.elion.redis.isSentinel()) {
            factory.setSentinelMaster(CC.elion.redis.sentinelMaster);
        }
        factory.init();
        test();
